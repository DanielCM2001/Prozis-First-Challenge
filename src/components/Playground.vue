...
<template>
  ...
  >
    ...
    <div v-if="balls.length === 0" class="space-y-5 text-center text-white">
      ...
      <div v-if="showExplanation" class="space-y-5" id="explanation_section">
...
// Reactive variables
const balls = ref([]); // Array to store created balls
let nextId = 1; // ID counter for new balls
let showExplanation = ref(false); // Flag to control the visibility of the explanation

// This function creates a new ball based on a click event
const createBall = (event) => {
  const size = getRandomSize(); // Generates a random size for the ball
  ...
  } while (color === "#f0f0f0"); // Ensure the color is not the same as the background, if it is, this will generate a new color again
...
/* Background image style applied conditionally */
.bg-playground-image {
  background: url("../assets/imgs/BallsBackground.jpg") no-repeat center center;
